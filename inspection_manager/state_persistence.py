...
class MissionStatePersistence:
    """SQLite persistence for mission and per-tire lifecycle."""
    ...
    def start_mission(self, object_id: str, config_json: str, allow_partial_success: bool = False) -> str:
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                """
                INSERT INTO events(mission_id, ts, state, event_type, payload_json)
                VALUES (?, ?, ?, ?, ?)
                """,
    ...
    def get_mission_state(self, mission_id: str) -> Optional[Dict[str, Any]]:
        with self._connect() as conn:
            ...
            if m is None:
                return None
            tires = conn.execute(
                "SELECT * FROM tires WHERE mission_id = ? ORDER BY order_index ASC",
                (mission_id,),
            ).fetchall()
            return {
                "mission": dict(m),
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                (
                    ...
                    status.upper(),
                    "tire_status",
                    json.dumps({"tire_id": tire_id, "status": status}),
                ),
            )

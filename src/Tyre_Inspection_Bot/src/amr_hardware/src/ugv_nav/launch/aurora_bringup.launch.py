...
def generate_launch_description():
    ...
    use_bridge_arg = DeclareLaunchArgument(
        "use_bridge",
        default_value="false",
        description="Legacy: enable aurora_sdk_bridge. Default false = Aurora 2.11 native depth (firmware 2.11+).",
    )
    stereo_camera_info_arg = DeclareLaunchArgument(
        "stereo_camera_info_enable",
        default_value="false",
        description="Enable Aurora factory camera_info (firmware 2.11+ may support). Try true after firmware upgrade.",
    )
    ...
    depth_optical2left = Node(
        ...
        arguments=['0', '0', '0', '0', '0', '0', '1', 'camera_left', 'camera_depth_optical_frame'],
    )

    bridge_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(get_package_share_directory("aurora_sdk_bridge"), "launch", "aurora_sdk_bridge.launch.py")
        ]),

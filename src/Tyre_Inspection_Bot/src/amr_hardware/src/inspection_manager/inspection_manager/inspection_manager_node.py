...
class VehicleInspectionManager(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("mission_log_path", "")  # if set, append JSONL log (mission_start, vehicle_detected, tire_photo, mission_end)
        ...
        # Mission log file path (empty = disabled)
        self._mission_log_path = self.get_parameter("mission_log_path").value
    ...
    def _set_state(self, new_state: str, cause: str = "unknown"):
        ...
        if new_state in [MissionState.WAIT_VEHICLE_BOX, MissionState.WAIT_TIRE_BOX]:
            ...
            self.wait_start_time = time.time()
            # Reset rotation attempts when starting fresh from IDLE or returning from rotation
            if prev_state == MissionState.IDLE or prev_state == MissionState.TURN_IN_PLACE_SEARCH:
                self.rotation_attempts = 0
        ...
        try:
            ...
            if (time.time() - self._tf_last_valid_time) > timeout:
                if not self._tf_watchdog_paused:
                    self.get_logger().error(
                        f"TF watchdog: slamware_map->base_link invalid > {timeout}s. Pausing mission."
                    )
    ...
    def _publish_mission_report(self):
        """Publish structured mission report (Phase L format)."""
    ...
    def _save_vehicle_position(self, box: BoundingBox3d):
        ...
        try:
            pose_map = self.tf_buffer.transform(pose_slam, "map", timeout=rclpy.duration.Duration(seconds=2.0))
        ...
        anchor_pose = {"frame_id": "map", "x": anchor_x, "y": anchor_y, "z": anchor_z, "yaw": anchor_yaw}
    ...
    def _process_vehicle_boxes(self, boxes: list):
        ...
        if self.current_state == MissionState.WAIT_VEHICLE_BOX:
            ...
            if vehicle_box:
                ...
                self._dispatch_box_goal(vehicle_box, offset=self._last_approach_offset)
    ...
    def _dispatch_box_goal(self, box: BoundingBox3d, offset: float):
        ...
        # Use tf_buffer.transform() - correct ROS 2 API for PoseStamped
        try:
            goal = self.tf_buffer.transform(
                goal_slamware, "map", timeout=rclpy.duration.Duration(seconds=2.0)
            )
            self.get_logger().info(
    ...
    def _get_current_pose_in_map_frame(self) -> Optional[PoseStamped]:
        ...
        try:
            return self.tf_buffer.transform(
                pose, "map", timeout=rclpy.duration.Duration(seconds=2.0)
            )
        except (TransformException, Exception):

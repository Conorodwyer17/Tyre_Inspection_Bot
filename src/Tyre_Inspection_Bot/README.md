...
- Detects vehicles via Aurora COCO80 semantic (car, truck, bus); tyres via YOLO best.pt
...
On Jetson, use the Jetson-optimized index for PyTorch:

```bash
pip3 install torch torchvision --extra-index-url https://pypi.jetson-ai-lab.io/jp6/cu126
```

If you see `libcusparseLt.so.0: cannot open shared object file`, install cuSPARSELt per Jetson docs and set `LD_LIBRARY_PATH` in `~/.bashrc`.

## Quick start

**Option 1: Mission launch (recommended)**
...
Runs pre-flight checklist then starts Aurora, motor driver, Nav2, detection pipeline, and inspection manager. Ensure Aurora is reachable at 192.168.11.1 and the motor UART is connected (default `/dev/ttyTHS1`).
...
- **Inspection manager:** Launch args – `vehicle_labels`, `tire_label` (use `car-tire`), `detection_topic`, `approach_offset`, `tire_offset`
...
Tyre_Inspection_Bot/
  src/amr_hardware/src/
    ...
    segment_3d/        # Aurora semantic fusion + YOLO tires + 3D boxes
...
Launch scripts: `~/ugv_ws/scripts/startup.sh`, `mission_launch.sh`, `aurora_pre_mission_checklist.sh`

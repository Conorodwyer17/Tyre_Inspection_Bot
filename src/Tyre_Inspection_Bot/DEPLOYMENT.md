...
ARCHITECTURE.md describes the hardware (chassis, ESP32, Jetson, Aurora), packages (ugv_nav, ugv_base_driver, segment_3d, message packages, ugv_vision, inspection_manager), data flow, TF/odom, and mission flow. The architecture matches the codebase.
...
| Run `nav_aurora.launch.py` without the full stack? | Start the Aurora SDK first (e.g. `aurora_bringup.launch.py`) so TF has `slamware_map`→`odom`→`base_link`. Nav2 needs the `map` frame (we publish `map`→`slamware_map`). |

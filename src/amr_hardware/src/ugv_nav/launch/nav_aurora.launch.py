...
def generate_launch_description():
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(bringup_dir, 'param', 'nav_aurora.yaml'),
    ...
    # ESP32 motor driver launch
    # Note: Update package name based on actual ESP32 driver package
    esp32_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('ugv_base_driver'), 'launch', 'esp32_driver.launch.py')
        )
    )
    
    # Robot pose publisher (if needed)
    ...
    bringup_cmd_group = GroupAction([
        ...
        # Localization only (Aurora provides map and odometry)
    ...
    ld.add_action(aurora_launch)
    ld.add_action(esp32_launch)
